...
echo ""
echo "=== Run PBRT camera render-time benchmark ==="
"$ross_root/.venv/bin/python" "$script_dir/test_pbrt_camera_render_times.py" \
  --run-benchmark \
  --pbrt "$ross_root/build/pbrt-v4/pbrt" \
  --output "$script_dir/runs/pbrt-camera-render-times" \
  --seed 1234 \
  --spp 1 \
  --spp 32 \
  "$@"
