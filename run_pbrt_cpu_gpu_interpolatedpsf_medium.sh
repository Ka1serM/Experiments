...
psf_json="$script_dir/assets/psfs/onsemi-ar0237-wide1-32x18-auto-rgb/onsemi-ar0237-wide1-32x18-auto-rgb-32x18.json"

if [[ -f "$psf_json" ]]; then
  echo "=== PSF already exists, skipping generation: $psf_json ==="
else
  echo "=== Step 1: Generate medium-density PSF (32x18 grid, 64 samples) ==="
...
"$ross_root/.venv/bin/python" "$script_dir/test_pbrt_cpu_gpu_e2e.py" \
  --scene "$script_dir/assets/scenes/slanted-edge-target/rossinterpolatedpsf_medium.pbrt" \
